...
describe('Notifications component', () => {
    ...
    it('calls markNotificationAsRead with the correct ID when a NotificationItem is clicked', () => {
        const markNotificationAsRead = jest.fn();
        const wrapper = mount(<Notifications displayDrawer={true} listNotifications={listNotifications} markNotificationAsRead={markNotificationAsRead} />);
        
        wrapper.find(NotificationItem).at(0).simulate('click');
        expect(markNotificationAsRead).toHaveBeenCalledWith(1);
    });

    it('updates the list of notifications when markNotificationAsRead is called', () => {
        const markNotificationAsRead = jest.fn((id) => {
            wrapper.setProps({
                listNotifications: listNotifications.filter(notification => notification.id !== id)
            });
        });
        const wrapper = mount(<Notifications displayDrawer={true} listNotifications={listNotifications} markNotificationAsRead={markNotificationAsRead} />);
